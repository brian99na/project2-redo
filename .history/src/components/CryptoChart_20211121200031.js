...
function CryptoChart(props) {
    ...
    const cryptoChartCalculator = () => {
        const mapData = chartData && chartData.map((day, index) => {
            let years = Math.floor(((chartData.length + 1) - index) / 365)
            let months = Math.floor((((chartData.length + 1) - index) % 365) / 30)
            let days = Math.floor((((chartData.length + 1) - index) % 365) % 30)
            const date = new Date()
            let todayValue = date.getDay() - days
            let todayMonthValue = date.getMonth() - months
            let todayYearValue = date.getFullYear() - years
